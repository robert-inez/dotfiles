...
-- Forces block cursor in all modes
vim.o.guicursor = ''
...
-- ── Column width + wrap per filetype ─────────────────────────────────────────
vim.api.nvim_create_autocmd('FileType', {
  group = vim.api.nvim_create_augroup('rinez-column-width', { clear = true }),
  pattern = {
    'typescript',
    'typescriptreact',
    'javascript',
  ...
  callback = function()
    ...
    local width = widths[ft] or 100
    vim.opt_local.textwidth = width
    vim.opt_local.colorcolumn = tostring(width)
    vim.opt_local.wrap = ft == 'markdown'
    vim.opt_local.linebreak = ft == 'markdown'
  end,
})

-- ── Treesitter manual start for non-standard filetypes ───────────────────────
-- Commented out — test if Kickstart's auto_install handles this automatically.
-- Uncomment if templ/go treesitter doesn't attach on its own.
-- vim.api.nvim_create_autocmd('FileType', {
--   group = vim.api.nvim_create_augroup('rinez-treesitter-start', { clear = true }),
--   pattern = { 'templ', 'go' },
--   callback = function() vim.treesitter.start() end,
-- })

-- ── Markdown: wrap, concealment, wikilinks, zk buffer keymaps ────────────────
vim.api.nvim_create_autocmd('FileType', {
  group = vim.api.nvim_create_augroup('rinez-markdown-zk', { clear = true }),
  pattern = 'markdown',
  callback = function()
    ...
    -- WikiLink and markdown link concealment
    vim.cmd [[
    ...
    -- zk buffer keymaps — only when inside a zk notebook
    local ok, zk_util = pcall(require, 'zk.util')
    if not ok then return end
    if zk_util.notebook_root(vim.fn.expand '%:p') == nil then return end

    local opts = { noremap = true, silent = false }
    local bmap = vim.api.nvim_buf_set_keymap
    bmap(0, 'n', '<CR>', '<Cmd>lua vim.lsp.buf.definition()<CR>', opts)
    bmap(0, 'n', 'K', '<Cmd>lua vim.lsp.buf.hover()<CR>', opts)
    bmap(0, 'n', '<leader>zb', '<Cmd>ZkBacklinks<CR>', opts)
    bmap(0, 'n', '<leader>zl', '<Cmd>ZkLinks<CR>', opts)
    bmap(0, 'n', '<leader>zi', '<Cmd>ZkInsertLink<CR>', opts)
    bmap(0, 'v', '<leader>znt', ":'<,'>ZkNewFromTitleSelection<CR>", opts)
  end,
})

vim.api.nvim_create_autocmd('BufWritePost', {
  pattern = '*.templ',
  callback = function() vim.fn.system 'templ generate' end,
})
...
require('lazy').setup({
  ...
  { -- Useful plugin to show you pending keybinds.
    ...
    opts = {
      ...
      spec = {
        ...
        { '<leader>z', group = '[Z]k Notes', mode = { 'n', 'v' } },
        { '<leader>zc', group = '[Z]k [C]reate from template', mode = { 'n' } },
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr) return { timeout_ms = 1000, lsp_format = 'fallback' } end,
      default_format_opts = {
